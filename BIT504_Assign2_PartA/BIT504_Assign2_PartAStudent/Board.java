...
public class Board {
	...
	public Board() {
	 ...
	 /** Return true if it is a draw (i.e no EMPTY cells) */ 
	public boolean isDraw() {
		//iterates through all cells
		for (int row = 0; row < GameMain.ROWS; ++row) {
			for (int col = 0; col < GameMain.COLS; ++col) {
				//if any cells are empty then returns false
				if (cells[row][col].getContent() == Player.Empty) {
					return false;
					}
				}
			}
		//if all cells are full, returns true
		return true;	
		}
	
	/** Return true if the current player "thePlayer" has won after making their move-done?  */
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		 // check if player has 3-in-that-row
		if(cells[playerRow][0].content == thePlayer && cells[playerRow][1].content == thePlayer && cells[playerRow][2].content == thePlayer )
			return true; 
		
		 // same as above but checking Col instead of row
